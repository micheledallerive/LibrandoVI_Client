...
export default class Adozioni extends React.Component{
    ...
    getSchools(increment){
        ...
        fetch("https://librandovi.it:3001"+query)
        .then((res)=>res.json())
        .then((data)=>{
            console.log(data.schools);
            console.log("Length: "+this.schoolsLength(data.schools)+" Count: "+data.count);
            this.setState({
                limit: newLimit,
        ...
        return(
            ...
            <div className="h-100 w-100">
                    ...
                    <div class="input-group w-75">
                      ...
                      <input type="text" class="form-control" style={{borderRadius: "0 .25rem .25rem 0"}} placeholder="Cerca scuola..." aria-label="ISBN" aria-describedby="basic-addon1" id="filter_input" onChange={(e)=>{
                            if(time_out){clearTimeout(time_out); time_out=null;}
                            time_out = setTimeout(()=>{
                                this.setState({filter: e.target.value, limit: 20}, ()=>this.getSchools(0));
                                time_out=null;
                            }, 500);
                            
                      }}/>
                      <button type="button" class="btn bg-transparent" style={{marginLeft: '-40px', zIndex: '10'}} onClick={()=>{this.setState({filter: "", limit: 20}, ()=>this.getSchools(0)); }}>
                    ...
                    <InfiniteScroll
                        ...
                        className="w-100 h-100 p-4">
                            {
                                    Object.keys(this.state.schools).reverse().map((value, index)=>(
                                        ...
                                        <>
                                            ...
                                            (
                                            <>
                                                <p className="text-secondary h4 mt-2">{this.state.schools[value].label}</p>
                                                <div className="row my-3">
                                                    {this.state.schools[value].schools.map((school, index)=>(
                                                        <>
