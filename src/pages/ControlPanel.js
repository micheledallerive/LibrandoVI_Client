...
export default class ControlPanel extends React.Component {
  ...
  addToMagazzino () {
    const data = $('#add_form').serializeArray()
  ...
  render () {
    const queryOk =
      queryString.parse(this.props.location.search)['?psw'] === 'asdrubale'
    return (
      <>
                ...
                <div className='container py-4'>
                  <div id='dbs'>
                    <div className='w-100 d-flex mb-3'>
                      ...
                      <button
                        ...
                        onClick={() => this.newDB(window.prompt('Nome:'))}
                      >
                        Nuovo
                      </button>
                    </div>
                    <ul class='list-group list-group-flush'>
                      {this.state.dbs.map((db, index) => (
                        <li
                          key={index}
                        ...
                        >
                          ...
                          <i
                            class='fas fa-trash d-flex flex-shrink-1 align-items-center pointer text-primary delete_db'
                            onClick={() => {
                  ...
                  <div id='magazzino' style={{ marginTop: 32 }}>
                    ...
                    <div className='magazzino_list mt-4'>
                      ...
                      >
                        ...
                        <tbody>
                          {this.state.magazzino.map((magItem, index) => (
                            <tr key={index}>
                              <td>{magItem.ISBN}</td>
                              <td>{magItem.quantity}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div className='ordini mt-4'>
                    ...
                    >
                      ...
                      <tbody>
                        {this.state.ordini.map((ordine, index) => (
                          <tr key={index}>
                            <td>{ordine._id}</td>
                            ...
                            <td>
                              {Object.keys(ordine.libri).map((key, index) => (
                                <p key={index}>
                                  {ordine.libri[key].action === 1
                                    ? 'Compra '
                                    : 'Vende '}
                                  {ordine.libri[key].quantity + ' '}
                                  {key}
                                </p>
                              ))}
                            </td>
